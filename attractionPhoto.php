...
<?php
	...
	$query = "SELECT attraction_photos.*, users.first_name, users.last_name FROM users NATURAL JOIN attraction_photos WHERE photo_id = '$photoID'"; 
	...
	$query = "SELECT * FROM users NATURAL JOIN attraction_photos"; 
	$result = mysqli_query($db, $query)or die("Error Querying Database");
	if($row = mysqli_fetch_array($result)) {
...
//used for the next/back buttons
	...
	$tryThis = $photoID + 1; 
	$query = "SELECT * FROM attraction_photos WHERE photo_id = '$tryThis' AND attraction_id='$attractionID'";
	//echo $query;
	...
	if (!empty($nextPhotoID)) {
		$nextLink = "<td align=\"right\" width=\"10%\" valign=\"top\" ><a href=\"attractionPhoto.php?id=" .  $nextPhotoID . "\"><h3><img src=\"next.png\" style=\"border:0px\"></h3></a></td>"; 
	}
	...
	if (!empty($previousPhotoID)) {
		$backLink = "<td align=\"left\" width=\"10%\" valign=\"top\" ><a href=\"attractionPhoto.php?id=" .  $previousPhotoID . "\"><h3><img src=\"back.png\" style=\"border:0px\"></h3></a></td>"; 
	}
...
//get the attraction photo comments
	//$query = "SELECT cip.*, cp.photo_id, u.user_id, u.first_name, u.last_name FROM attraction_photos cp NATURAL JOIN attraction_photo_comments cip NATURAL JOIN users u WHERE cp.photo_id = $photoID ORDER BY cip.comment_date_submitted DESC";
	$query = "SELECT cip.*, u.user_id, u.first_name, u.last_name FROM attraction_photo_comments cip NATURAL JOIN users u WHERE cip.photo_id = $photoID ORDER BY cip.comment_date_submitted DESC";
...
<form action="attractionPhotoCommentSubmitted.php" method="post" class="form">
<center>
<table>
