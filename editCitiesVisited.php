...
<form action=editCitiesSubmit.php method="POST" >
<?php
	...
	//get cities the user has already selected
	$query = "SELECT city_id FROM userCities WHERE user_id = " . $user_id;
	...
	while($row = mysqli_fetch_array($result)) {
		...
		//output city names and pics as checkbox options for which cities the user has visited with previously selected cities selected
